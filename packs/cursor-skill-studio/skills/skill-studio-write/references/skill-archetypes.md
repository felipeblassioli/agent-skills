...
Merged from the archetype-selection (writing-cursor-skills) and
archetype-patterns (create-skill-from-refs) references.
...
The three skill-archetype templates live in
...
the distinguishing patterns of each exemplar so you can adapt them in place.

### 1. Knowledge Hub — exemplar `tdd-classicist`
...
### 2. Tool Runner — exemplar `test-verifier`

Rich frontmatter for discoverability:
...
Delegation guidance (Cursor-specific):
...
Output contract — exact markdown the skill produces:
...
scripts are the core value.

### 3. Workflow Executor — exemplar `gh-pr-creator`

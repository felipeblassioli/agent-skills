...
def load_run_results(benchmark_dir: Path) -> dict:
    ...
    Returns dict keyed by configuration name, each containing a list of run
    results.
    """
                ...
                for exp in raw_expectations:
                    if "text" not in exp or "passed" not in exp:
                        print(
                            ...
                            f"{grading_file} missing required fields (text, passed, evidence): {exp}"
                        )
...
def generate_benchmark(benchmark_dir: Path, skill_name: str = "", skill_path: str = "") -> dict:
    ...
    benchmark = {
        "metadata": {
            "skill_name": skill_name or "<artifact-name>",
            "skill_path": skill_path or "<path/to/artifact>",
...
def generate_markdown(benchmark: dict) -> str:
    ...
    lines.append(
        ...
        f"{b_tokens.get('mean', 0):.0f} ± {b_tokens.get('stddev', 0):.0f} | {delta.get('tokens', '—')} |"
    )

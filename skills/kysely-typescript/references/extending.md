...
Let's expand the `JsonValue` example from the [previous section](https://kysely.dev/docs/recipes/#expression). We'll add an `as` method for the `JsonValue` class that can be used to turn an `Expression<T>` into an `AliasedExpression<T, A>`:

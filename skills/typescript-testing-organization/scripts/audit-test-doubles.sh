...
# audit-test-doubles.sh — Scan TS test files for mock/spy usage and flag overuse.
...
# Find all test files (both .spec and .test for migration detection)
test_files=$(find "$PROJECT_ROOT" \
  -type f \( -name "*.spec.ts" -o -name "*.spec.tsx" -o -name "*.test.ts" -o -name "*.test.tsx" \) \
  ! -path "*/node_modules/*" \
  ! -path "*/.cursor/*" \
  ! -path "*/dist/*" \
  2>/dev/null | sort)

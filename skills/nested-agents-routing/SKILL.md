...
| `packs/` | Installable Cursor runtime assets | `.cursor/agents`, `.cursor/rules`, hooks, MCP examples, templates, guides; optional **bundled skills** (`packs/<name>/skills/...` + `kind: "skill"` in `pack.json`) that install to `.cursor/skills/<skillId>/` or `~/.cursor/skills/<skillId>/` |
...
5. Use a `pack` only when you need installable `.cursor/` runtime assets,
   reusable templates and guides, and/or **pack-bundled skills** that should
   deploy with the same install (see `docs/specs/agentic-skill-pack-authoring.md`).
...
- packs provide runtime assets and templates (and optional bundled skills), not
  repo-specific nested `AGENTS.md` files

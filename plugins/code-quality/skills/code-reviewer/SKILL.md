...
- **Don't infer absence from a partial diff.** If a finding claims something is *missing* or *not implemented* (an error type, handler, helper, guard) and that code would live *outside the shown diff*, verify it in the repo — `grep` / `Read` the file — before flagging it. A summarized or truncated diff is not proof the code is absent. Unconfirmed absence is a `QUESTION`, never a `BLOCKER`. Conversely, when a mock or fixture supplies data the real request never asks for (e.g. a test that hand-feeds a response the API only returns under an explicit `expand`/param), a passing test is not proof — flag the gap.

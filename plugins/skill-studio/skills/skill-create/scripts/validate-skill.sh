...
# Validates an Agent Skill directory for structural correctness against the
# Claude-first contract (SKILL.md frontmatter = name + description only;
# governance/freshness lives in metadata.json). Returns JSON with pass/fail
# results and any findings. Self-contained: no repo-specific paths.
#
# This is the local structural checker bundled with the skill. The repository
# also ships scripts/validate-skill.sh at its root, which additionally checks
# metadata.json, CHANGELOG.md, and registry consistency; run that before a PR.
set -euo pipefail
...
# --- Legacy/governance fields do not belong in frontmatter (Claude contract) ---
for legacy in version last_reviewed source_contracts compatibility disable-model-invocation; do
  if echo "$frontmatter" | grep -qE "^${legacy}:"; then
    add_warning "SKILL.md frontmatter has field '$legacy'; keep frontmatter to name + description (governance goes in metadata.json)"
  fi
done
...
# Skip URLs, anchors, and ${CLAUDE_*} runtime variables.

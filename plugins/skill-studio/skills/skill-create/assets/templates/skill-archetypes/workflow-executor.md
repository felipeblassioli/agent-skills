...
- INVARIANT_4 (e.g., "Confirm before writing files")
...
- The quick reference table is a cheat sheet to avoid re-reading the full
  workflow for common actions.
- If a step has conditional paths, use a sub-section with a situation table.
- If the workflow touches assets (templates, config), store them in `assets/`
  and reference from the relevant step. Invoke bundled scripts via
  `${CLAUDE_SKILL_DIR}`.
